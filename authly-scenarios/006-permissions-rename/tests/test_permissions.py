...
def test_evaluate_returns_true_when_role_grants_permission():
    ...
    assert client.permissions.evaluate(user_id=user.id, permission="documents:read") is True
    assert client.permissions.evaluate(user_id=user.id, permission="documents:write") is True


def test_evaluate_raises_when_permission_missing():
    client, user = _client_with_user()
    role = client.roles.create(name="viewer", permissions={"documents:read"})
    client.roles.assign(user_id=user.id, role_id=role.id)

    with pytest.raises(AuthorizationError) as excinfo:
        client.permissions.evaluate(user_id=user.id, permission="documents:delete")

    assert user.id in str(excinfo.value)
    assert "documents:delete" in str(excinfo.value)


def test_evaluate_raises_for_unknown_user():
    client = Authly(project_id="proj", api_key="key")

    with pytest.raises(AuthorizationError):
        client.permissions.evaluate(user_id="usr_unknown", permission="documents:read")


def test_evaluate_is_case_sensitive():
    client, user = _client_with_user()
    role = client.roles.create(name="editor", permissions={"documents:write"})
    client.roles.assign(user_id=user.id, role_id=role.id)

    with pytest.raises(AuthorizationError):
        client.permissions.evaluate(user_id=user.id, permission="documents:Write")
