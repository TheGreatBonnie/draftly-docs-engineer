...
def test_client_raises_for_empty_project_id():
    with pytest.raises(ValueError, match="project_id is required"):
        Authly(project_id="", scoped_token="tok")


def test_client_raises_for_missing_scoped_token():
    with pytest.raises(ValueError, match="scoped_token is required"):
        Authly(project_id="proj", scoped_token="")


def test_api_key_parameter_was_removed():
    with pytest.raises(TypeError):
        Authly(project_id="proj", api_key="legacy_key")  # type: ignore[call-arg]
...
def test_client_stores_configuration():
    client = Authly(project_id="proj_demo", scoped_token="demo_token")

    assert client.project_id == "proj_demo"
    assert client.scoped_token == "demo_token"


def test_client_raises_when_no_credential_is_supplied():
    with pytest.raises(ValueError, match="scoped_token is required"):
