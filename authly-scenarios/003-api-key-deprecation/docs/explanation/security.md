...
- Scoped tokens are accepted without validation beyond non-emptiness.
- Tokens are random strings with expiry metadata only; no signing, revocation lists, or rotation.
- OAuth supports authorization-code exchange; PKCE is not implemented.
...
- Use placeholder values (`proj_demo`, `tok_demo`) in examples and tests.
