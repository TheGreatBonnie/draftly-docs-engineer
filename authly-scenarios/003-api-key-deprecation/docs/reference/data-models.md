...
| `organization_id`| str \| None | Organization the role is scoped to; `None` means unscoped |

Because `permissions` and `assignments` are sets, duplicate inserts are no-ops and membership tests are O(1). Roles created without `organization_id` are unscoped and considered in every organization scope.
