...
Installing the package registers an `authly` console script (see `[project.scripts]` in `pyproject.toml`). The CLI is intentionally minimal and performs no network or state operations.
