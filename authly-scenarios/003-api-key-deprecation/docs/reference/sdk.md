...
The installed version is available as `authly.__version__` (`2.0.0`).
...
| `scoped_token` | str  | yes      | Empty value raises `ValueError`.             |

Both `project_id` and `scoped_token` are keyword-only. The legacy `api_key` parameter was removed in v2.0.0.
...
| `authly.oauth`       | `OAuthClient`        | Authorization URL construction and code exchange |
