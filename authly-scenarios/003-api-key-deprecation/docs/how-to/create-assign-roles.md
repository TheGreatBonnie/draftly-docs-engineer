...
Roles can be scoped to an organization by passing `organization_id`:
...
Scoped roles only satisfy permission checks restricted to that organization (plus unscoped checks). See [Manage organizations](manage-organizations.md).
