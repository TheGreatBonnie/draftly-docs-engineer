...
**Cause:** `project_id` or `scoped_token` was omitted or empty.

**Fix:** see [Configure the client](configure-client.md).

## Symptom: OAuth flow stops after the redirect

**Cause:** the authorization code was not exchanged. `authorization_url()` only builds the authorization URL; the code returned by the redirect must be exchanged explicitly. PKCE is not supported.

**Fix:** exchange the code for an access token:

```python
token = authly.oauth.exchange_code(code="the-code-from-the-redirect")
```
